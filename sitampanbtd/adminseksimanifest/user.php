...
//$nourut=1;
?>
<form method="post" action=<?php echo "index.php?hal=pageadminman&pilih=user"; ?> >
<input name="katakunci" type="text" value="<?php echo "$katakunci"; ?>" class="required" />
<input class="button putih bigrounded " type="submit" name="Submit2" value="Cari" class="submitsearch" /> <span>*) Nama dan NIP</span> 
...
$reload = "index.php?hal=pageadminman&pilih=user";

// data mentah 
$sql = "SELECT iduser, username,level,nm_lengkap,nip_baru,jabatan,status_user FROM user where ((nm_lengkap LIKE '%$katakunci%') 
OR (nip_baru LIKE '%$katakunci%')) and (level='manifest' or level='seksimanifest' or level='adminmanifest') and status_user='aktif' ";
...
OR (nip_baru LIKE '%$katakunci%')) and (level='manifest' or level='seksimanifest' or level='adminmanifest') and status_user='aktif'  LIMIT $limit,$rpp";
...
// data mentah 
$sql = "SELECT * FROM user where (level='manifest' or level='seksimanifest' or level='adminmanifest') and status_user='aktif' ";
$result = mysql_query($sql);
...
// counting the offset
$limit = ($nohal - 1) * $rpp;
$sqltampil = "SELECT * FROM user where (level='manifest' or level='seksimanifest' or level='adminmanifest') and status_user='aktif' LIMIT $limit,$rpp" ;
...
<td align="center" class="isitabel"><a href="?hal=useredit&id=<?php echo  $data['iduser']; ?>"><img src="images/new/update.png" title="edit"/></a> | <a href="?hal=pageadminman&pilih=userdel&id=<?php echo  $data['iduser']; ?>"><img src="images/new/del.gif"   title="Delete User"/></a>
