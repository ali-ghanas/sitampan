...
//    $tahun=$_GET['tahun'];
    ...
    $title = "<font size=5 face=arial><b>Daftar BCF 1.5 Yang Belum / Tidak dapat ditarik ke TPP (Belum ada BA Penarikan)</b></font>";
    $title1 = "";
    ...
    $sql = "SELECT * FROM bcf15,tpp,seksi where ((bcf15no LIKE '%$in_katakunci%') 
                                    OR (blno LIKE '%$in_katakunci%') OR (consignee LIKE '%$in_katakunci%') OR (notify LIKE '%$in_katakunci%')) and (bcf15tgl between '$in_tglawal' and '$in_tglakhir') and bcf15.idtpp LIKE '%$in_tpp%' and recordstatus='2' and bcf15.idtpp=tpp.idtpp and bcf15.idseksi=seksi.idseksi and masuk='2' and batal='2'  ";
    $q   = mysql_query( $sql );
    ...
    header("Content-Disposition: attachment; filename=tdk_masuk_tpp.xls");
