...
include 'lib/function.php';
	...
	$sql = "SELECT idbcf15,tahun,bcf15no,bcf15tgl,bc11no,bc11tgl,bc11pos,bc11subpos,
        blno, bltgl, saranapengangkut, bcf15.idmanifest,manifest.idmanifest,idtps,idtpp,consignee,notify,kd_manifest FROM bcf15,manifest WHERE bcf15.idmanifest=manifest.idmanifest and bcf15no=$id and tahun=$tahun"; // memanggil data dengan id yang ditangkap tadi
...
<div id="kotakdetail">
       <form id="cont" name="cont" action="" method="post">
        ...
        <td align="center"><input size="5" align="center" class="required" id="no" type="text" name="no[0]" value="0"/></td>       
    ...
    '<td align="center"><input size="5" class="required" type="text" id="no" name="no['+i+']" value="'+i+'"/></td>'+
    ...
    <span>Kolom Ukuran Kontainer diisi tanpa tanda petik (') </span>
</div> 
  <?php
    session_start();
    ...
    if(isset($_POST['addcont'])) // jika tombol addsubmit ditekan
	{
               ...
               if ($query) {
                    ...
                    echo '<script type="text/javascript">window.location="index.php?hal=pagemanifest&pilih=addbcf15"</script>';
                }
