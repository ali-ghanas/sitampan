...
<form method="post" id="tpsadd" name="tpsadd" action="?hal=pageadmin&pilih=addtps">
<table border="0">
    <tr align="center"> 
...
if(isset($_POST['addsubmit'])) {
    ...
    $alamat = $_POST['alamat'];
    $kdtps = $_POST['kdtps'];
    $sql = "SELECT * FROM tps where nm_tps='$nama'";
    // var_dump($sql); exit(0);
    ...
    // }elseif(strlen($kdtps)<4){
    //     echo '<script type="text/javascript">
    //     alert("KD TPS harus 6 digit");</script>';
    }else {
        mysql_query("INSERT INTO tps(nm_tps, alamat_tps, kd_tps)
        VALUES('$nama', '$alamat', '$kdtps')");
        echo '<script type="text/javascript">window.location="index.php?hal=pageadmin&pilih=addtps"</script>';
    }
}; // if(kondisi) {hasil};
?>
</body>
