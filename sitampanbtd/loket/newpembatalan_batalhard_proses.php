...
<html>
           ...
           $(document).ready(function() {    
              $("#ndkonfirmasi").submit(function() {
                 ...
                 if ($.trim($("#konf_stsnhi").val()) == "NHI") {
                    alert("<?php session_start(); echo " $_SESSION[nm_lengkap]..." ?> Konfirmasi Penindakan menyatakan dokumen ini adalah NHI");
                    $("#konf_stsnhi").focus();
                    return false;  
                 }
                ...
                $update_konf = mysql_query("UPDATE kofirmasi_p2 SET
                 ...
                 if($update_konf){
                    ...
                    if($nohp_pemohon==''){
                        ...
                        echo "<table bgcolor=#202750><tr><td><img src=images/new/warning.png /></td><td><font size=5 color=#fff>Pembatalan BCF 1.5 telah tersimpan. (Aplikasi SMS tidak menemukan No HP) </font></td></tr></table></br>";
                    echo "<a href=?hal=newpembatalan&pilihloket=new_batalhard><<-Inbox Pembatalan Hardcopy->></a> | <a href='report/doc/pembatalanbcf15.php?id=$id'><<-Cetak Pembatalan Preview->></a> | <a href='report/kartukendalikonfirmasi.php?id=$id'><<-Cetak Kartu Kendali Konfirmasi->></a>  "; 
                    }
                    else{
                    ...
                    echo "<a href=?hal=newpembatalan&pilihloket=new_batalhard><<-Inbox Pembatalan Hardcopy->></a> | <a href='report/doc/pembatalanbcf15.php?id=$id'><<-Cetak Pembatalan Preview->></a> | <a href='report/kartukendalikonfirmasi.php?id=$id' target=_blank><<-Cetak Kartu Kendali Konfirmasi->></a> "; 
                 }
                 }
                 else {
	...
	$bcf15 = mysql_fetch_array($query);
        ...
        ?>
        <form method="post" id="ndkonfirmasi" name="ndkonfirmasi" action="?hal=newpembatalan&pilihloket=new_batalhard_proses">
