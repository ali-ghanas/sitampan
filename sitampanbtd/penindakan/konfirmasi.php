                   ...
                   <?php // yang belum koneksi database diabaikan aja dulu file ini
                    ...
                    $cek1=mysql_numrows($query1);
                            ...
                            $update_konf = mysql_query("UPDATE kofirmasi_p2 SET
                                  ...
                                  echo "<table bgcolor=#339cdf><tr><td><font face=verdana color=#fff><b>Segera balas Konfirmasi di bawah ini karena pada pukul 12.00 WIB konfirmasi yang dikirim sebelum tanggal  $sekarang dan belum dijawab akan dikirim konfirmasi hardcopy nya oleh Seksi Tempat Penimbunan</b></font></td></tr></table></br>";
                            } 
               
                    
                    
//                    $tanggal = "2013-05-10"; // tgl yang akan dicari nama harinya
//
...
//
//                    echo "Pada tanggal <b>$tanggal</b> adalah hari <b>$namahari</b>";
//
//                    
                    ...
                    <form method="post" action=<?php echo "index.php?hal=daftarconf&pilihp2=newkonf"; ?> >
                    <table border="0" >
                            ...
                            <td><b>
                                <table class="data isitabel" bgcolor="#fafbea">
                                    <tr><td colspan="2"><b>Pilih Kriteria Pencarian BCF 1.5</b></td></tr>
                                     ...
                                     <tr>
                                      <td><input type="checkbox" name="Pib" value="1"  <?php  if($_POST['Pib'] == 1){echo 'checked="checked"';}?>>No Pendaftaran/Pemberitahuan </td>
                                     <td><input type="text" size="10" class="required" name="pib" value="<?php echo $_POST['pib']?>"> </td>
                                     
                                     <td><input type="checkbox" name="Pengeluaran" value="1"  <?php  if($_POST['Pengeluaran'] == 1){echo 'checked="checked"';}?>>No Persetujuan Pengeluaran  </td>
                                     <td><input type="text" size="10" class="required" name="pengeluaran" value="<?php echo $_POST['pengeluaran']?>"></td> 
                                     </tr>
                                       ...
                                       if (isset($_POST['Pib']))
                                        {
                                           ...
                                           if (empty($bagianWhere))
                                           {
                                                $bagianWhere .= "DokumenNo like '%$pib%'";
                                           }
                                           else
                                           {
                                                $bagianWhere .= " AND DokumenNo like '%$pib%'";
                                           }
                                        }
                                       if (isset($_POST['Pengeluaran']))
                                        {
                                           ...
                                           if (empty($bagianWhere))
                                           {
                                                $bagianWhere .= "Dokumen2No like '%$pengeluaran%'";
                                           }
                                           else
                                           {
                                                $bagianWhere .= " AND Dokumen2No like '%$pengeluaran%'";
                                           }
                                        }
                    ...
                    $reload = "index.php?hal=daftarconf&pilihp2=newkonf";

                    // data mentah ,
                    $sql = "SELECT kofirmasi_p2.idbcf15,konf_bcf15no,konf_bcf15tgl,konf_tahun,konf_bc11no,konf_bc11tgl,konf_bc11pos,konf_bc11psubpos,konf_tglkonftpp,nm_usertpp,nip_usertpp,ip_usertpp,konf_statusakhir,consignee,notify,DokumenCode,DokumenNo,DokumenDate,Dokumen2Code,Dokumen2No,Dokumen2Date,idtpp
                        FROM kofirmasi_p2,bcf15 where bcf15.idbcf15=kofirmasi_p2.idbcf15 and   (konf_statusakhir='konf_system') and ".$bagianWhere."   ";
                    ...
                    // counting the offset
                    $limit = ($nohal - 1) * $rpp;
                    $sqltampil = "SELECT kofirmasi_p2.idbcf15,konf_bcf15no,konf_bcf15tgl,konf_tahun,konf_bc11no,konf_bc11tgl,konf_bc11pos,konf_bc11psubpos,konf_tglkonftpp,nm_usertpp,nip_usertpp,ip_usertpp,konf_statusakhir,consignee,notify,DokumenCode,DokumenNo,DokumenDate,Dokumen2Code,Dokumen2No,Dokumen2Date,idtpp
                        FROM kofirmasi_p2,bcf15 where bcf15.idbcf15=kofirmasi_p2.idbcf15 and  (konf_statusakhir='konf_system' ) and ".$bagianWhere."    LIMIT $limit,$rpp";
                    ...
                    // data mentah 
                    $sql = "SELECT kofirmasi_p2.idbcf15,konf_bcf15no,konf_bcf15tgl,konf_tahun,konf_bc11no,konf_bc11tgl,konf_bc11pos,konf_bc11psubpos,konf_tglkonftpp,nm_usertpp,nip_usertpp,ip_usertpp,konf_statusakhir,consignee,notify,DokumenCode,DokumenNo,DokumenDate,Dokumen2Code,Dokumen2No,Dokumen2Date,idtpp
                        FROM kofirmasi_p2,bcf15 where bcf15.idbcf15=kofirmasi_p2.idbcf15 and   (konf_statusakhir='konf_system' )   ";
                    $result = mysql_query($sql);
                    ...
                    $limit = ($nohal - 1) * $rpp;
                    $sqltampil = "SELECT kofirmasi_p2.idbcf15,konf_bcf15no,konf_bcf15tgl,konf_tahun,konf_bc11no,konf_bc11tgl,konf_bc11pos,konf_bc11psubpos,konf_tglkonftpp,nm_usertpp,nip_usertpp,ip_usertpp,konf_statusakhir,consignee,notify,DokumenCode,DokumenNo,DokumenDate,Dokumen2Code,Dokumen2No,Dokumen2Date,idtpp
                        FROM kofirmasi_p2,bcf15 where bcf15.idbcf15=kofirmasi_p2.idbcf15 and   (konf_statusakhir='konf_system' )   LIMIT $limit,$rpp" ;
                   ...
                   <br/>
                    ...
                    <table class="data" >
                            ...
                            <tr>
                                ...
                                <th class="judultabel">No Pemberitahuan</th>
                    ...
                    while($data = mysql_fetch_array($tampil)){
                        if($data['konf_statusakhir']=='konf_hardcopy') {$status='Kirim Hardcopy';} elseif($data['konf_statusakhir']=='konf_system') {$status='Konf BCF 1.5 by sistem';} elseif($data['konf_statusakhir']=='konf_jawabok') {$status='Konf dijawab by System';} elseif($data['konf_statusakhir']=='konf_selesai') {$status='Konfirmasi Selesai';} 
                    ...
                    <td class="isitabel">Pemberitahuan :<?php
                                                            $sql = "SELECT * FROM dokumen ORDER BY iddokumen";
                                                            $qry = @mysql_query($sql) or die ("Gagal query");
                                                            while ($datadok1 =mysql_fetch_array($qry)) {
                                                                if ($datadok1[iddokumen]==$data[DokumenCode]) {
                                                    
                                                       
                                                                echo "$datadok1[dokumenname]";
                                                                }
                                                            }
                                                        ?>
                                            <br/> 
                                         Nomor : <?php echo  $data['DokumenNo']; ?> <br/>
                                         Tanggal : <?php echo  $data['DokumenDate']; ?> <br/>
                    </td>
                    <td class="isitabel"><?php 
                                                            $sql = "SELECT * FROM tpp  ORDER BY idtpp";
                                                            $qry = @mysql_query($sql) or die ("Gagal query");
                                                            while ($datatpp =mysql_fetch_array($qry)) {
                                                                if ($datatpp[idtpp]==$data[idtpp]) {
                    ...
                    <td class="isitabel"><a href="?hal=daftarconf&pilihp2=jwb_konf&id=<?php echo $data['idbcf15'] ?>">Jawab</a></td>
