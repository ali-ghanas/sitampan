...
$order=trim($order); if (!empty($order) || $order!="") { $orderby=" order by ".$order." "; }
...
if ($modeturbo==0) {
	...
	if ($select=="*") { $selectnumrows="count(*) as rjml"; }
	$jml=mysql_query("select ".$selectnumrows." from ".$tname.$ha.$groupby);
	$rjmlarray=mysql_fetch_array($jml);
	$rjml=$rjmlarray["rjml"];
}
...
if (empty($carrierselectffswtabel) || $carrierselectffswtabel=="") { $carrierselectffswtabel="ffswtabel"; }

/* KE HALAMAN AWAL */
if ($page>1) {
	$first="<div onClick=\"funffdivmain('".$filefetcher."?ffswitch=".$carrierselectffswtabel."&page=1&limit=".$limit."&limitawal=0&".$carrier."&sortlist=".$sortlist."&orderlist=".$orderlist."','')\">&laquo;Awal</div>";
}
...
if ($page>1) {
	...
	$prev="<div onClick=\"funffdivmain('".$filefetcher."?ffswitch=".$carrierselectffswtabel."&page=".$prevpage."&limit=".$limit."&limitawal=".$prevlimit."&".$carrier."&sortlist=".$sortlist."&orderlist=".$orderlist."','')\">&#8249;Sebelum</div>";
}
else { $prev="&#8249;Sebelum"; }
...
if ($jmlhal>0&&$page<$jmlhal) {
	...
	$next="<div onClick=\"funffdivmain('".$filefetcher."?ffswitch=".$carrierselectffswtabel."&page=".$nextpage."&limit=".$limit."&limitawal=".$nextlimit."&".$carrier."&sortlist=".$sortlist."&orderlist=".$orderlist."','')\">Sesudah&#8250;</div>";
}
else { $next="Sesudah&#8250;"; }
...
if ($jmlhal>1&&$page<$jmlhal) {
	...
	$last="<div onClick=\"funffdivmain('".$filefetcher."?ffswitch=".$carrierselectffswtabel."&page=".$lastpage."&limit=".$limit."&limitawal=".$lastlimit."&".$carrier."&sortlist=".$sortlist."&orderlist=".$orderlist."','')\">Akhir&raquo;</div>";
}
...
$sort=mysql_query("select ".$select.$count." from ".$tname.$ha.$groupby.$orderby." limit ".$limitawal.",".$limitakhir);
?>