    ...
    <div class="span12">
        <div class="span4">
            <div class="nav-header">
                <h5>History Akses User </h5>
            </div>
        </div>
        <div class="span8" style="left:auto">
            
            <form class="form-search" method="post" action=<?php echo "index.php?hal=user&pilih=manajemenuserhist"; ?> >
...
$reload = "index.php?hal=user&pilih=manajemenuserhist";

// data mentah 
$sql = "SELECT * FROM user,log where log.tanggal between '$tglawal' and '$tglakhir' and log.username=user.username   order by log.tanggal and jamout desc  ";
...
// counting the offset
$limit = ($nohal - 1) * $rpp;
$sqltampil = "SELECT * FROM user,log where log.tanggal between '$tglawal' and '$tglakhir' and log.username=user.username   order by log.tanggal and jamout desc  LIMIT $limit,$rpp";
...
// data mentah 
$sql = "SELECT * FROM user,log where log.username=user.username order by log.tanggal desc   ";
$result = mysql_query($sql);
...
// counting the offset
$limit = ($nohal - 1) * $rpp;
$sqltampil = "SELECT * FROM user,log where log.username=user.username order by log.tanggal  desc   LIMIT $limit,$rpp" ;
